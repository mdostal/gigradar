...
`v0.32.0` — real source adapters (LinkedIn, GoFractional, A.Team, Wellfound, Braintrust, and more, plus LLM-driven custom sources and Gmail digest ingestion), a real **Dashboard** (metrics/last-scan overview) split from the **All Gigs** giglist (per-column filters, tier/status/source/profile-match/rate/seen-window, sticky and actually usable), a **Config dashboard** — card-grid home + collapsible sidebar + a full page per section (Profile/Sources/Groups/Schedule/Automation/Appearance) — multi-group support with per-group tiering/scoring, an opt-in AI-verification pass that catches keyword false positives (and automatically re-tiers/archives stale unseen gigs), guided browser-session login, assisted-apply drafting with a career-tracker/interview-prep layer, an MCP server for Claude Desktop/Code, a cron scheduler with desktop notifications, a dynamic per-source sync-status dropdown, a severity-tiered issues view with an in-UI human-verification co-pilot, and an opt-in, trust-gated auto-fire submission system. See [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) for the full design and build-out roadmap. Try it live at [mdostal.github.io/gigradar](https://mdostal.github.io/gigradar/).
