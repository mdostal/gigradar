...
## Part 5 — Decision points — RESOLVED by the owner, 2026-09-01

1. **Tier semantics across multiple matched groups**: **per-group tier**, confirmed. `matched_group_tiers: Record<groupId, Tier>` replaces the flat `tier` column's role as the per-group source of truth — see Part 1g below (added after this resolution).
2. **Default group's placeholder label**: `"Default Search 1"` (numbered, generic, safe against guessing wrong on real data) for the ONE-TIME migration path specifically (Slice 1 — no LLM call involved, a non-interactive migration). Separately, the owner wants group CREATION generally (Slice 2's "add a group" UI) to attempt an LLM-assisted auto-fill of the label/criteria when an LLM credential is available — noted as a real Slice 2 UX requirement, not in scope for Slice 1's migration path itself.
3. **Sequencing**: confirmed — Slice 1 ships and gets live-verified against the real database alone, before Slice 2 (Config UI) starts.

### Part 1g — Tier becomes per-group (added after Decision Point 1)

Revises Part 1a/1c/1d above: `matched_group_ids` (Part 1c) is joined by a second nullable JSON column, `matched_group_tiers TEXT` (JSON-stringified `Record<string, Tier>` — e.g. `{"fractional-search": "green", "drone-stuff": "yellow"}`), populated by `matchGroups()` (Part 1f) recording each group's OWN `tier(gig, group.roleArea)` result alongside its pass/fail gate result. The existing flat `tier` column and `Gig.tier`/`StoredGig.tier` field are UNCHANGED for backward compatibility (still reflects tier against the FIRST group in `Config.groups` order, or the single implicit group for a not-yet-multi-group install) — Slice 3 (dashboard UI)'s per-group views read `matched_group_tiers[thisGroup]` instead of the flat field once a gig is actually being viewed inside a specific group's dashboard.
