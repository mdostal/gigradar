...
- [x] Graduated auto-fire trust system (`graduated-auto-fire-trust` epic) — real, per-`(sourceId, tier)` submission automation, off by default and earned rather than flipped on: `Config.autoFire` (a global `killSwitch` plus per-pair `{enabled, minApprovals, dailyCap}` rules), `src/lib/apply/autofire.ts`'s `approvedCount()`/`isGraduated()` (pure computation over real `application_drafts` approval history — never a separate, driftable counter) and `evaluateAutoFire()` (the full 6-step decision tree: kill switch → rule exists → enabled → graduated → a `SubmitAdapter` is registered → the 4 default checks — tier=green, draft-content sanity, freshness/not-delisted, daily fire cap), an append-only `autofire_decisions` audit table (every evaluation logged, fired or not, with the rule snapshot in effect at decision time), a `SubmitAdapter` registry (`src/lib/submit/adapter.ts`, mirrors `Source`'s registration pattern) with a double-submit safety net (`application_drafts.status` gains `'submitting'`, `markDraftSubmitting()`/`markDraftFailed()`), scheduler wiring (`runAutoDraft()` attempts auto-fire right after a successful draft, fully isolated per-gig try/catch), and a `/config` Auto-fire section (kill switch, rule editor, on-demand "N/minApprovals approved" trust-status check). **The first real `SubmitAdapter` (GoFractional) is paused, not shipped** — live-verified with the owner directly that GoFractional's job-detail page still shows a Cloudflare verification challenge even from a real, authenticated, human-worked session; "no `SubmitAdapter` registered" is the correct, safe, fully-tested terminal state every pair reaches today. See `.pHive/epics/graduated-auto-fire-trust/stories/gofractional-submit-adapter.yaml`'s `pause_note` for the live-verified finding and a likely real fix (a persistent, console-attached browser session, structurally referenced from the owner's prior tool) flagged for a future story.
- [x] Severity-tiered issue signaling (`notifications-epic`) — a durable, deduped `issues` store (`src/lib/notify/issues.ts`: `raiseIssue()`/`resolveIssue()`/`listIssues()`, `warning`/`error`, dedupe-on-`(source, title)` against already-open issues only, reusing `src/lib/notify/desktop.ts`'s existing best-effort desktop notification unmodified) wired into three real, already-existing failure paths — the scheduler's per-source fetch errors, `runAutoDraft()`'s drafting failures (both `warning`), and `attemptAutoFire()`'s evaluation/submit failures (`warning`/`error` — a real submission attempt failing is a materially bigger deal) — plus a `/issues` dashboard page and a nav badge (`src/app/issues-badge.ts`, pure/tested: hidden at zero open, amber for warnings-only, red whenever any error is open). This is the general "flag it, don't fail silently" mechanism the owner asked for after the GoFractional Cloudflare-verification finding above — the paused submit adapter itself will report through this once it resumes. Also fixed a real regression this epic's own testing caught live: several pre-existing scheduler tests were silently writing fixture rows to the real production database once `runAutoDraft()`'s new call site gave their store-unaware error mocks a real side effect — `vitest.setup.ts` now sets a global throwaway `XDG_DATA_HOME` default (not `GIGRADAR_DB_PATH`, which would have conflicted with `src/mcp/__tests__/server.test.ts`'s own child-process isolation) so no test can touch the owner's real database again, even incidentally.
- [x] True 1-click macOS installer + real auto-update (`tauri-installer` epic, 4 stories) — a third runtime mode alongside browser/Electron: a real, double-clickable `.app`/`.dmg` (`src-tauri/`). `tauri-bare-packaged-app`: the Tauri shell spawns a bundled, PINNED Node sidecar (never Node-SEA/`pkg`-compiled — native-binding robustness for `node:sqlite`/Playwright, a deliberate deviation from Tauri's own official sidecar tutorial, documented in `src-tauri/src/lib.rs`) running gigradar's `.next/standalone` server, polls for real TCP readiness, and only then opens a window — same "no window before the server is confirmed up" discipline as "Two runtime modes" below. `tauri-chromium-sidecar`: Playwright's Chromium bundled too, zero code changes needed in `browser-session.ts` (`PLAYWRIGHT_BROWSERS_PATH` transparently respected, live-verified). `tauri-ci-release-pipeline`: `.github/workflows/tauri-release.yml` — pushing a `vX.Y.Z` (prod) or `vX.Y.Z-dev.N` (dev channel) tag builds and publishes a `.dmg` to GitHub Releases via `tauri-action`. **Two distinct "signing" concepts, easy to conflate:** the update BUNDLE carries a real Tauri-updater ed25519 signature (the local test keypair described below, verified by `tauri-plugin-updater` before applying an auto-update) — that part is real and working. The `.app`/`.dmg` itself is **not** Apple-code-signed or notarized (no Apple Developer ID certificate configured) — it's ad-hoc signed only, so macOS Gatekeeper still blocks first launch with the standard "unidentified developer" dialog; `xattr -cr /Applications/gigradar.app` clears the quarantine flag and lets it run (or right-click → Open once from Finder). Real Apple signing/notarization is a later, owner-gated step (needs the owner's own Apple Developer Program enrollment), same class of gate as Google OAuth client credentials — not something this repo/assistant can self-serve. Documented for installers in README.md's Install section, `docs/index.html`'s Install step, and every release's own notes (`tauri-release.yml`'s `releaseBody`).

**`macos-adhoc-codesign` fix (2026-08-17, shipped in v0.24.1):** through v0.24.0, `bundle.macOS` had no `signingIdentity` configured at all, so tauri-bundler never ran `codesign` over the finished `.app` — the ONLY signature present was the bare per-binary linker-adhoc signature Xcode/Rust's linker auto-applies to every arm64 Mach-O (a hard requirement on Apple Silicon), which carries no bundle-level `_CodeSignature/CodeResources` seal at all. `codesign --verify --deep --strict` failed outright on that ("code has no resources but signature indicates they must be present"), and macOS surfaced this as **"gigradar.app is damaged and can't be opened"** — Gatekeeper correctly detecting a genuinely malformed signature, not merely an absent one, and NOT offering the usual bypass. Setting `bundle.macOS.signingIdentity: "-"` makes tauri-bundler run a real `codesign --deep` ad-hoc pass over the whole bundle after all resources are staged, producing a proper resource seal (live-verified against the actual v0.24.1 `.dmg`: `codesign --verify --deep --strict` now returns "valid on disk, satisfies its Designated Requirement" with `Sealed Resources version=2 rules=13 files=3065`, vs. `Sealed Resources=none` before). This does **not** add a real Developer ID signature — `spctl -a` still (correctly) rejects it, same as any non-notarized app — so the `xattr -cr` step above is still required; the fix only downgrades the failure mode from an unrecoverable "damaged, move to Trash" dialog to the standard, bypassable "unidentified developer" one.

**`dynamic-server-port` fix (2026-08-17):** both `src-tauri/src/lib.rs` and `electron/main.ts` hardcoded `127.0.0.1:3000` for the spawned/bundled server with no override, so any other local service already holding port 3000 (a common collision — plenty of dev tooling defaults to it) made gigradar fail to start outright, no workaround short of freeing the port. Both now call a `resolve_server_port()`/`resolvePort()` helper (`src-tauri/src/lib.rs`, `electron/server-ready.ts`) that PREFERS port 3000 (or a `GIGRADAR_PORT` env override) — so a stable, once-registered Gmail OAuth redirect URI (see below) keeps working across launches — but falls back to an OS-assigned free port (bind to port 0, read back what the OS assigned, drop the listener; small accepted TOCTOU race between releasing that probe socket and the real server binding it, same class of risk this codebase already accepts elsewhere, e.g. `session-capture.ts`'s sanity-check-before-write) rather than refusing to start. A fallback logs/dialogs a warning, since it breaks Gmail OAuth for that session (see `docs/gmail-oauth-setup.md`'s `GIGRADAR_PORT` escape hatch). `src/lib/auth/oauth2.ts`'s `REDIRECT_BASE_URL` was also de-hardcoded — it now reads the running server's own `process.env.PORT` (set by whichever of the above spawned it) rather than a bare `"http://127.0.0.1:3000"` literal, so the OAuth redirect URI always matches whatever port the server actually bound to.

**`port-free-check-connect-not-bind` fix (2026-08-17, shipped in v0.25.1):** the `dynamic-server-port` fix above still shipped a real bug — both `is_port_free()` (`src-tauri/src/lib.rs`) and `isPortFree()` (`electron/server-ready.ts`) checked whether a port was free by attempting to **bind** it. Live-verified this gives a false positive on macOS against a real, already-listening third-party process: `TcpListener::bind()` sets `SO_REUSEADDR` on Unix by Rust's own std default, and `net.Server.listen()` behaves the same way by Node's own default — both succeeded binding a port a real, unrelated, already-running local service was actively listening on. Concretely, this meant the packaged app could believe its preferred port was free, hand it to its own sidecar, and build its window pointed at that port — which was still actually answering as the OTHER service, so the gigradar window loaded that service's page instead of gigradar's own. Root-caused by directly binding against a real conflicting listener on a real machine and observing the false `Ok` result, not guessed at. Fixed by switching both checks to a **connect** attempt instead: a live listener always accepts a connection, a genuinely free port always refuses one (`ECONNREFUSED`) — verified correct against the same real conflicting listener before shipping. Rust unit tests added (`src-tauri/src/lib.rs`'s `#[cfg(test)] mod tests`) reproducing the exact regression with a real `TcpListener`, so it can't silently regress again.

**`macos-jit-entitlements` fix (2026-08-17, shipped in v0.25.1):** a SEPARATE, more severe bug discovered while live-verifying the fix above against the actual CI-built, ad-hoc-signed `.dmg` (not a local unsigned build, which had been masking this): the bundled Node sidecar crashed immediately on launch with `SIGTRAP`/`EXC_BREAKPOINT` inside V8's own initialization (`pthread_jit_write_protect_np`, inside `v8::internal::ThreadIsolation::Initialize` — confirmed via the real macOS crash report at `~/Library/Logs/DiagnosticReports/node-*.ips`, not guessed). Root cause: the `macos-adhoc-codesign` fix above (which started running `codesign` over the whole bundle) means every binary in the bundle is now a signed, hardened-runtime-eligible process — and on this platform, a signed process without the right entitlements is blocked from toggling W^X memory permissions, which is exactly what V8's JIT needs to do at startup. Fixed by adding `src-tauri/entitlements.plist` (`com.apple.security.cs.allow-jit`, `com.apple.security.cs.allow-unsigned-executable-memory`, `com.apple.security.cs.disable-library-validation` — the last also needed for the bundled `node:sqlite`/Playwright native addons to `dlopen` under the same hardened-runtime restriction) and wiring it via `bundle.macOS.entitlements` in `tauri.conf.json`. Live-verified against a real locally-built, fully-signed `.app` (`codesign --verify --deep --strict` passing, entitlements confirmed present via `codesign -d --entitlements -`): the sidecar now starts and serves real content on its resolved port, where before it crashed before ever listening on anything.

`tauri-real-auto-update`: `tauri-plugin-updater` wired entirely from the Rust side (`src-tauri/src/updater.rs`) — a native tray menu ("Check for Updates", "Update channel: Dev/Prod", "Quit"), checking on launch and on demand. Channel preference is a small sibling file next to gigradar's own data dir (installer-level state, not `Config`), defaulting to prod. Prod's endpoint uses GitHub's own "latest release" alias (only resolves non-prerelease tags); dev's endpoint is a `latest.json` kept current on a dedicated `dev-manifest` branch the release workflow overwrites, since GitHub has no "latest prerelease" equivalent — the actual signed update bundle stays a normal, permanent release asset, only that pointer needs to be overwritable. Live-verified end to end: a real installed build detected and applied a real published update, channel preference intact across the update-triggered restart. Signed with a local test keypair (`npx tauri signer generate`) for now — the real production signing key is a later, owner-gated step (Portunus-vaulted).
...
`start` script always carries, plus `PORT` — see below), polls the resolved
server URL on a bounded retry loop until it responds, and only then opens a
`BrowserWindow` pointed at it — never loads before the server is actually
ready. This sidesteps ever needing to know whether Electron's own bundled
Node honors `--experimental-sqlite` the same way a plain `node` invocation
does: Electron's own runtime never touches `node:sqlite` at all, only the
spawned child (the system's own Node) does. `electron/server-ready.ts` holds
the polling AND port-resolution logic as a small, Electron-free module,
unit-tested in isolation against a real `node:http`/`node:net` server
standing in for the spawned child (`electron/__tests__/server-ready.test.ts`)
— no real Electron process needed for that test.

Port selection prefers 3000 (or a `GIGRADAR_PORT` override) but falls back to
an OS-assigned free port when that's already bound by something else (e.g. a
concurrent `npm run dev`, or an unrelated local service) — see the
`dynamic-server-port` fix entry above. If something is STILL listening on
the freshly-resolved port right after resolution (a rare race — see that
entry), a clear native dialog is shown ("port already in use...") instead of
hanging indefinitely or failing silently. On window close/app quit
(`window-all-closed` AND `before-quit`, not just the happy path), the
...
would otherwise orphan a process still squatting on its port for the next
launch.
...
leaves no orphaned `next-server` process behind, and launching with port
