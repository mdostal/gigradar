...
["a.team", "platform.a.team"] }`). Both `gofractional.ts`/`ateam.ts`
...
- [ ] Remaining real `Source` adapters: fractional-jobs, wellfound, gun.io.
...
(`src/lib/sources/origins.ts` — currently `gofractional` and `ateam`),
`config-client.tsx` renders a "Capture login" button.
