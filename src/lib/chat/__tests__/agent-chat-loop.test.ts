...
describe("sendMessage: plain conversational turn, no tools", () => {
  it("returns the model's final text answer when it calls no tools", async () => {
    ...
    const result = await sendMessage("s1", { kind: "api-key", value: "fake-api-key" }, "hello");
  ...
  it("throws a specific error when sendMessage() is called for a session that was never started", async () => {
    await expect(sendMessage("never-started", { kind: "api-key", value: "fake-api-key" }, "hello")).rejects.toThrow(/no chat session/);
  });
});
...
describe("sendMessage: list_gigs tool", () => {
  it("calls the REAL listGigs() and grounds the model's next turn in real data", async () => {
    ...
    startChatSession("s2");
    const result = await sendMessage("s2", { kind: "api-key", value: "fake-api-key" }, "how many green-tier gigs do I have?");
...
describe("sendMessage: get_gig tool", () => {
  it("returns a real gig's data, BEGIN/END-delimited as untrusted DATA", async () => {
    ...
    startChatSession("s3");
    await sendMessage("s3", { kind: "api-key", value: "fake-api-key" }, "tell me about this gig");
  ...
  it("returns a clean tool error (not a thrown exception) for an unknown key", async () => {
    ...
    startChatSession("s4");
    const result = await sendMessage("s4", { kind: "api-key", value: "fake-api-key" }, "tell me about gig does-not:exist");
...
describe("sendMessage: get_status_summary tool", () => {
  it("calls the REAL computeStatusStrip() and reflects real store/config state", async () => {
    ...
    startChatSession("s5");
    const result = await sendMessage("s5", { kind: "api-key", value: "fake-api-key" }, "what's my status?");
...
describe("sendMessage: read-only tools never mutate the store", () => {
  it("gigs.db row content is unchanged after a multi-turn read-only conversation", async () => {
    ...
    startChatSession("s6");
    await sendMessage("s6", { kind: "api-key", value: "fake-api-key" }, "look everything up");
...
describe("sendMessage: multi-turn conversation retains history", () => {
  it("a follow-up message's request includes the earlier turns' history", async () => {
    ...
    startChatSession("s7");
    await sendMessage("s7", { kind: "api-key", value: "fake-api-key" }, "hi");

    mockCreate.mockResolvedValueOnce(fakeTextResponse("As I said, ask away."));
    await sendMessage("s7", { kind: "api-key", value: "fake-api-key" }, "still there?");
...
describe("sendMessage: MAX_TURNS", () => {
  it("stops cleanly with turn_limit_reached rather than looping forever when the model keeps calling tools", async () => {
    ...
    startChatSession("s8");
    const result = await sendMessage("s8", { kind: "api-key", value: "fake-api-key" }, "keep going forever");
...
describe("write tools: propose then approve, no exceptions", () => {
  it("update_gig_status produces a proposal, never mutates before approval", async () => {
    ...
    startChatSession("w1");
    const result = await sendMessage("w1", { kind: "api-key", value: "fake-api-key" }, "mark this one applied");
  ...
  it("sendMessage() throws if called again while a proposal is still pending", async () => {
    ...
    mockCreate.mockResolvedValueOnce(fakeToolUseResponse("update_gig_status", { key, status: "applied" }));
    startChatSession("w2");
    await sendMessage("w2", { kind: "api-key", value: "fake-api-key" }, "mark this one applied");

    await expect(sendMessage("w2", { kind: "api-key", value: "fake-api-key" }, "another message")).rejects.toThrow(/still awaiting approval/);
    endChatSession("w2");
  });
  ...
  it("resolveApproval(approve:true) executes the REAL setStatus() and the loop continues to a final message", async () => {
    ...
    startChatSession("w3");
    await sendMessage("w3", { kind: "api-key", value: "fake-api-key" }, "mark this one applied");
    const result = await resolveApproval("w3", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
  ...
  it("resolveApproval(approve:false) never touches the store, and the loop continues", async () => {
    ...
    startChatSession("w4");
    await sendMessage("w4", { kind: "api-key", value: "fake-api-key" }, "mark this one applied");
    const result = await resolveApproval("w4", { kind: "api-key", value: "fake-api-key" }, false, FAKE_CONFIG);
  ...
  it("resolveApproval() throws when there is no pending approval", async () => {
    startChatSession("w5");
    await expect(resolveApproval("w5", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG)).rejects.toThrow(/no pending approval/);
    endChatSession("w5");
  });
  ...
  it("generate_draft: approval calls the REAL stageApplication() with the gig/config/apiKey", async () => {
    ...
    startChatSession("w6");
    await sendMessage("w6", { kind: "api-key", value: "fake-api-key" }, "draft an application for this one");
    await resolveApproval("w6", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);

    expect(stageApplicationMock).toHaveBeenCalledWith(expect.objectContaining({ gig: expect.objectContaining({ sourceId: "src-a" }) }), FAKE_CONFIG, { kind: "api-key", value: "fake-api-key" });
    endChatSession("w6");
  });
  ...
  it("generate_prep_packet: approval calls the REAL generatePrepPacket() and persists via saveInterviewPrep()", async () => {
    ...
    startChatSession("w7");
    await sendMessage("w7", { kind: "api-key", value: "fake-api-key" }, "generate a prep packet for this one");
    const result = await resolveApproval("w7", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
  ...
  it("run_scan: approval calls the REAL runRadar() with the config and BYOK apiKey", async () => {
    ...
    startChatSession("w8");
    await sendMessage("w8", { kind: "api-key", value: "fake-api-key" }, "run a scan");
    const result = await resolveApproval("w8", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
  ...
  it("a failed write tool (e.g. unknown gig key) surfaces as a tool error to the model, never throws out of resolveApproval()", async () => {
    ...
    startChatSession("w9");
    await sendMessage("w9", { kind: "api-key", value: "fake-api-key" }, "mark does-not:exist as applied");
    const result = await resolveApproval("w9", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
...
describe("source-connection tools: propose then approve, no exceptions", () => {
  it("start_capture_login produces a proposal, never opens a browser before approval", async () => {
    ...
    startChatSession("c1");
    const result = await sendMessage("c1", { kind: "api-key", value: "fake-api-key" }, "log me into gofractional");
  ...
  it("start_capture_login: approval calls the REAL startCapture() with the registry login URL, and sets chat-owned activeCapture state", async () => {
    ...
    startChatSession("c2");
    await sendMessage("c2", { kind: "api-key", value: "fake-api-key" }, "log me into gofractional");
    const result = await resolveApproval("c2", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
  ...
  it("finish_capture_login: no-op error (not a throw out of resolveApproval) when this chat never started a capture", async () => {
    ...
    startChatSession("c3");
    await sendMessage("c3", { kind: "api-key", value: "fake-api-key" }, "finish the login");
    const result = await resolveApproval("c3", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
  ...
  it("finish_capture_login (local backend): approval calls the REAL finishCapture() against the chat-owned capture, and persists sessionStatePath to config.json", async () => {
    ...
    startChatSession("c4");
    await sendMessage("c4", { kind: "api-key", value: "fake-api-key" }, "log me into gofractional");
    await resolveApproval("c4", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
    await sendMessage("c4", { kind: "api-key", value: "fake-api-key" }, "I'm done, finish it");
    const result = await resolveApproval("c4", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
  ...
  it("a second start_capture_login while one is already open still lets take_screenshot/finish/cancel act on the chat's single activeCapture (no cross-session lookup)", async () => {
    ...
    startChatSession("c5");
    await sendMessage("c5", { kind: "api-key", value: "fake-api-key" }, "log me into gofractional");
    await resolveApproval("c5", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
    await sendMessage("c5", { kind: "api-key", value: "fake-api-key" }, "never mind, cancel it");
    const result = await resolveApproval("c5", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
  ...
  it("start_gmail_connect: approval returns the REAL authorization URL from buildAuthorizationUrl()", async () => {
    ...
    startChatSession("c6");
    await sendMessage("c6", { kind: "api-key", value: "fake-api-key" }, "connect my gmail alerts source");
    const result = await resolveApproval("c6", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
  ...
  it("disconnect_gmail: approval calls the REAL deleteTokenSet() for the source", async () => {
    ...
    startChatSession("c7");
    await sendMessage("c7", { kind: "api-key", value: "fake-api-key" }, "disconnect gmail-alerts");
    const result = await resolveApproval("c7", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
...
describe("take_screenshot: read-only, auto-executes against the chat-owned capture", () => {
  it("returns a clean tool error (not a thrown exception) when no capture is open in this chat", async () => {
    ...
    startChatSession("s9");
    const result = await sendMessage("s9", { kind: "api-key", value: "fake-api-key" }, "show me the login window");
  ...
  it("takes a real screenshot of the chat's own active capture and feeds it back as an image tool_result", async () => {
    ...
    await sendMessage("s10", { kind: "api-key", value: "fake-api-key" }, "log me into gofractional");
    await resolveApproval("s10", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
    const result = await sendMessage("s10", { kind: "api-key", value: "fake-api-key" }, "what does it look like?");
...
describe("list_source_presets: read-only, auto-executes", () => {
  it("never appears as a pendingApproval and returns SOURCE_PRESETS' id/label/description", async () => {
    ...
    startChatSession("p1");
    const result = await sendMessage("p1", { kind: "api-key", value: "fake-api-key" }, "what job sites can you add?");
...
describe("add_source: propose then approve, no exceptions", () => {
  ...
  it("with a presetId produces a proposal, never mutates config.json before approval", async () => {
    ...
    const result = await sendMessage("as1", { kind: "api-key", value: "fake-api-key" }, "add zoho recruit");
  ...
  it("approving a presetId proposal writes a SourceConfig matching sourceConfigFromPreset()'s own output", async () => {
    ...
    await sendMessage("as2", { kind: "api-key", value: "fake-api-key" }, "add zoho recruit");
    const result = await resolveApproval("as2", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
  ...
  it("approving a suggestsGmailDigest:true preset mentions Gmail/start_gmail_connect in the tool_result text; a preset without the flag does not", async () => {
    ...
    await sendMessage("as3", { kind: "api-key", value: "fake-api-key" }, "add zoho recruit");
    await resolveApproval("as3", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
    ...
    await sendMessage("as4", { kind: "api-key", value: "fake-api-key" }, "add welcome to the jungle");
    await resolveApproval("as4", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
  ...
  it("start_gmail_connect is NOT called automatically as a side effect of approving add_source", async () => {
    ...
    await sendMessage("as5", { kind: "api-key", value: "fake-api-key" }, "add indeed");
    await resolveApproval("as5", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
  ...
  it("with a raw sourceId+url (no preset) writes a custom-llm SourceConfig with a uniqued id on collision", async () => {
    ...
    await sendMessage("as6", { kind: "api-key", value: "fake-api-key" }, "add monster at https://example.test/monster/jobs");
    await resolveApproval("as6", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
  ...
  it("rejecting an add_source proposal never touches config.json", async () => {
    ...
    await sendMessage("as7", { kind: "api-key", value: "fake-api-key" }, "add indeed");
    const result = await resolveApproval("as7", { kind: "api-key", value: "fake-api-key" }, false, FAKE_CONFIG);
  ...
  it("an unknown presetId surfaces as a clean tool error, not a thrown exception out of resolveApproval()", async () => {
    ...
    await sendMessage("as8", { kind: "api-key", value: "fake-api-key" }, "add does-not-exist");
    const result = await resolveApproval("as8", { kind: "api-key", value: "fake-api-key" }, true, FAKE_CONFIG);
