...
type Row = { company: string; title: string; statusLabel: string; updatedText: string };
...
describe("reconcileWellfoundStatuses", () => {
  it("updates a locally-tracked gig's status when an ONGOING row maps 'Pending' -> applied", async () => {
    ...
    stubTwoPages([{ company: "Acme", title: "Founding CTO", statusLabel: "Pending", updatedText: "Aug 26" }], []);
  ...
  it("updates a locally-tracked gig's status when an ARCHIVED row maps 'Expired' -> archived", async () => {
    ...
    stubTwoPages([], [{ company: "Thoughtful AI", title: "Software Engineer, Platform", statusLabel: "Expired", updatedText: "Oct 23, 2024" }]);
  ...
  it("merges rows from BOTH pages in one result", async () => {
    ...
    seedGig("2-b", "Role B");
    stubTwoPages(
      [{ company: "X", title: "Role A", statusLabel: "Pending", updatedText: "1 day" }],
      [{ company: "Y", title: "Role B", statusLabel: "Expired", updatedText: "2 days" }],
    );
  ...
  it("reports alreadyCurrent and does not re-write when the local status already matches", async () => {
    seedGig("1-a", "Role A");
    stubTwoPages([{ company: "X", title: "Role A", statusLabel: "Pending", updatedText: "1 day" }], []);
  ...
  it("reports noMatch (never fabricates a match) for a row with no corresponding local gig", async () => {
    stubTwoPages([{ company: "Some Co", title: "A Role Gigradar Never Tracked", statusLabel: "Pending", updatedText: "1 day" }], []);
  ...
  it("reports ambiguous (never guesses) when a row's title normalizes to MORE THAN ONE local gig", async () => {
    ...
    stubTwoPages([{ company: "X", title: "Founding CTO", statusLabel: "Pending", updatedText: "1 day" }], []);
  ...
  it("reports unknownStatusLabel for a status badge text this mapping doesn't recognize", async () => {
    seedGig("1-a", "Founding CTO");
    stubTwoPages([{ company: "X", title: "Founding CTO", statusLabel: "Some Brand New Label", updatedText: "1 day" }], []);
